...
//  }
  ...
  test("Marker.Composition.WithTrace") {
    val marker = Marker() withSymbol("circle") withColors("red") withLine(Line())

    val x_axis = Axis(key = "xaxis", title = "x variable")
    val y_axis = Axis(key = "yaxis", title = "y variable")

    val series = XY(x_int, y_int, series_type = SCATTER, series_mode = MARKERS) withMarker  marker

    val layout = Layout("Marker.Composition.WithTrace") withAxes(x_axis, y_axis)

    val chart = Chart() withSeries series withLayout layout
...
class AnimationTests extends AnyFunSuite {
  test("Animation.XY") {
    ...
    val layout = Layout("Animation.XY")  withAxes (xaxis, yaxis)
    val traces = createXYSeries(numberToCreate = 50, length=30)
    val chart = Chart(animated=true) withLayout layout withSeries traces
    if (true) chart.plot()
