...
final case class AnimationEngine(animation_mode: String = "immediate", transition_duration: Double = 300.0,
  ...
  private val play_btn_args =
    Args(mode = animation_mode, fromcurrent = true, transition = Setting(duration = transition_duration),
      frame = Setting(duration = transition_duration, redraw = true))

  private val play_button = PlayButton(method = "animate", args = play_btn_args, label = play_btn_label)

  private val pause_btn_args = Args(mode = animation_mode, fromcurrent = false, transition = Setting(duration = 0.0),
    frame = Setting(duration = 0.0, redraw = false))

  private val pause_button = PauseButton(method = "animate", args = pause_btn_args, label = pause_btn_label)

  /** can be combined with the layout */
  val update_menus =
    UpdateMenus(x = btn_x, y = btn_y, yanchor = btn_y_anchor, xanchor = btn_x_anchor,
