...
final case class Chart(data: List[Series] = Nil, layout: Layout = Layout(), config: Config = Config(),
  ...
  def plot_inline()(implicit publish: OutputHandler): Unit = plotChart_inline(data_, layout_, config_)
  ...
  private def genAnimatedLayout(): Value = {
